...
public class RegistroServiceImpl implements RegistroService {
    ...
    public Model activar(Model model, String username, String clave) {
        User usuario
                = usuarioService.getUserPorUsernameYPassword(username,
                        clave);
        if (usuario != null) {
    ...
    @Override
    public Model crearUser(Model model, User usuario)
            throws MessagingException {
        String mensaje;
        if (!usuarioService.
                existeUserPorUsernameOCorreo(
                        usuario.getUsername(),
                        usuario.getCorreo())) {
    ...
    @Override
    public Model recordarUser(Model model, User usuario)
            throws MessagingException {
        String mensaje;
        User usuario2 = usuarioService.getUserPorUsernameOCorreo(
                usuario.getUsername(),
                usuario.getCorreo());
        if (usuario2 != null) {
            String clave = demeClave();
